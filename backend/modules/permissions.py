...
# ══════════════════════════════════════════════════════════════════════════
# OWASP MOBILE TOP 10 (2024) DATABASE
# ══════════════════════════════════════════════════════════════════════════

OWASP_MOBILE_TOP10 = {
    "M1": {
        "id": "M1", "title": "Improper Credential Usage",
        "description": "Hardcoded credentials, insecure credential storage or transmission.",
    },
    "M2": {
        "id": "M2", "title": "Inadequate Supply Chain Security",
        "description": "Third-party libraries and SDKs may introduce unaudited vulnerabilities.",
    },
    "M3": {
        "id": "M3", "title": "Insecure Authentication/Authorization",
        "description": "Weak authentication mechanisms allow unauthorized access to sensitive data.",
    },
    "M4": {
        "id": "M4", "title": "Insufficient Input/Output Validation",
        "description": "Missing input validation enables injection and data manipulation attacks.",
    },
    "M5": {
        "id": "M5", "title": "Insecure Communication",
        "description": "Data transmitted without encryption or proper certificate validation.",
    },
    "M6": {
        "id": "M6", "title": "Inadequate Privacy Controls",
        "description": "Excessive data collection without consent or clear purpose limitation.",
    },
    "M7": {
        "id": "M7", "title": "Insufficient Binary Protections",
        "description": "Lack of code obfuscation, tamper detection, or anti-reverse engineering.",
    },
    "M8": {
        "id": "M8", "title": "Security Misconfiguration",
        "description": "Debug flags enabled, exported components, or insecure default settings.",
    },
    "M9": {
        "id": "M9", "title": "Insecure Data Storage",
        "description": "Sensitive data stored on-device without encryption, accessible to attackers.",
    },
    "M10": {
        "id": "M10", "title": "Insufficient Cryptography",
        "description": "Weak cryptographic algorithms, hardcoded keys, or improper implementation.",
    },
}

# Permission → OWASP Mobile Top 10 mapping
PERMISSION_OWASP_MAP = {
    # M1 — Improper Credential Usage
    "android.permission.GET_ACCOUNTS":               ["M1", "M6"],
    # M2 — Inadequate Supply Chain Security
    "android.permission.REQUEST_INSTALL_PACKAGES":   ["M2", "M8"],
    "android.permission.UPDATE_PACKAGES_WITHOUT_USER_ACTION": ["M2", "M8"],
    # M3 — Insecure Authentication/Authorization
    "android.permission.USE_BIOMETRIC":              ["M3"],
    "android.permission.USE_FINGERPRINT":            ["M3"],
    # M5 — Insecure Communication
    "android.permission.INTERNET":                   ["M5"],
    "android.permission.ACCESS_NETWORK_STATE":       ["M5"],
    "android.permission.ACCESS_WIFI_STATE":          ["M5"],
    "android.permission.CHANGE_WIFI_STATE":          ["M5"],
    "android.permission.CHANGE_NETWORK_STATE":       ["M5"],
    "android.permission.NFC":                        ["M5"],
    # M6 — Inadequate Privacy Controls
    "android.permission.ACCESS_FINE_LOCATION":       ["M6"],
    "android.permission.ACCESS_COARSE_LOCATION":     ["M6"],
    "android.permission.ACCESS_BACKGROUND_LOCATION": ["M6"],
    "android.permission.CAMERA":                     ["M6"],
    "android.permission.RECORD_AUDIO":               ["M6"],
    "android.permission.READ_CONTACTS":              ["M6", "M9"],
    "android.permission.WRITE_CONTACTS":             ["M6", "M9"],
    "android.permission.READ_SMS":                   ["M6"],
    "android.permission.SEND_SMS":                   ["M6"],
    "android.permission.RECEIVE_SMS":                ["M6"],
    "android.permission.READ_CALL_LOG":              ["M6", "M9"],
    "android.permission.WRITE_CALL_LOG":             ["M6", "M9"],
    "android.permission.READ_PHONE_STATE":           ["M6"],
    "android.permission.READ_PHONE_NUMBERS":         ["M6"],
    "android.permission.READ_CLIPBOARD":             ["M6"],
    "android.permission.QUERY_ALL_PACKAGES":         ["M6", "M8"],
    "android.permission.POST_NOTIFICATIONS":         ["M6"],
    "android.permission.BLUETOOTH_SCAN":             ["M6"],
    # M8 — Security Misconfiguration
    "android.permission.SYSTEM_ALERT_WINDOW":        ["M8"],
    "android.permission.WRITE_SETTINGS":             ["M8"],
    "android.permission.BLUETOOTH_CONNECT":          ["M8"],
    "android.permission.MANAGE_OWN_CALLS":           ["M8"],
    # M9 — Insecure Data Storage
    "android.permission.READ_EXTERNAL_STORAGE":      ["M9"],
    "android.permission.WRITE_EXTERNAL_STORAGE":     ["M9"],
    "android.permission.MANAGE_EXTERNAL_STORAGE":    ["M8", "M9"],
    "android.permission.ACCESS_MEDIA_LOCATION":      ["M6", "M9"],
    "android.permission.READ_MEDIA_IMAGES":          ["M9"],
    "android.permission.READ_MEDIA_VIDEO":           ["M9"],
    "android.permission.READ_MEDIA_AUDIO":           ["M9"],
}
...
def analyze_permissions(app_id: str) -> dict:
    ...
    for name in raw_perms:
        ...
        permissions_out.append({
            ...
            "risk":   risk,
            "label":  label,
            "owasp":  PERMISSION_OWASP_MAP.get(name, []),
        })
...
def _owasp_severity(owasp_id: str, evidence_count: int) -> str:
    """Derive severity from OWASP category and evidence volume."""
    high_risk = {"M1", "M2", "M6", "M9"}
    if owasp_id in high_risk:
        return "high" if evidence_count >= 3 else "medium"
    return "medium" if evidence_count >= 2 else "low"


def get_owasp_assessment(permissions: list, tracker_names: list = None) -> dict:
    """
    Build an OWASP Mobile Top 10 assessment from permissions and detected SDKs.

    Args:
        permissions  : list of full android.permission.* strings
        tracker_names: list of SDK/tracker display names (e.g. "Facebook SDK")

    Returns:
        {
          "findings"      : list of finding dicts,
          "top_risks"     : list of triggered OWASP IDs,
          "risk_count"    : int,
          "total_categories": 10,
        }
    """
    tracker_names = tracker_names or []
    triggered: dict = {}  # owasp_id → list[str evidence]

    # Map permissions → OWASP categories
    for perm in permissions:
        for owasp_id in PERMISSION_OWASP_MAP.get(perm, []):
            triggered.setdefault(owasp_id, [])
            triggered[owasp_id].append(perm.split(".")[-1])

    # All embedded SDKs → M2 (Supply Chain)
    if tracker_names:
        triggered.setdefault("M2", [])
        for name in tracker_names:
            triggered["M2"].append(f"SDK: {name}")

    # Ad/analytics SDKs also raise M6 (Privacy Controls)
    ad_analytics = [n for n in tracker_names
                    if any(k in n.lower() for k in ("ad", "analytics", "tracking", "tiktok", "bytedance"))]
    if ad_analytics:
        triggered.setdefault("M6", [])
        for name in ad_analytics:
            triggered["M6"].append(f"Tracker: {name}")

    findings = []
    for owasp_id in sorted(triggered.keys()):
        evidence = list(dict.fromkeys(triggered[owasp_id]))  # dedup, preserve order
        info = OWASP_MOBILE_TOP10.get(owasp_id, {})
        findings.append({
            "id":          owasp_id,
            "title":       info.get("title", owasp_id),
            "description": info.get("description", ""),
            "severity":    _owasp_severity(owasp_id, len(evidence)),
            "evidence":    evidence[:6],
        })

    return {
        "findings":         findings,
        "top_risks":        list(triggered.keys()),
