...
def build_pdf(report: dict) -> bytes:
    ...
    if owasp_findings:
        story.append(Paragraph(
            '<font name="Helvetica-Bold" size="12" color="#1A1A18">07 — OWASP Mobile Top 10 Assessment</font>',
            s("h2"),
        ))
        story.append(Spacer(1, 1 * mm))
        risk_count = owasp.get("risk_count", 0)
        story.append(Paragraph(
            f'<font size="9" color="#6A6A66">'
            f'{risk_count} of 10 OWASP Mobile Top 10 (2024) categories triggered. '
            f'Analysis based on declared permissions and embedded SDKs.</font>',
            s("owasp_intro"),
        ))
        story.append(Spacer(1, 3 * mm))

        sev_color = {"high": _TERRA, "medium": _STONE, "low": _OLIVE}
        owasp_header = [
            Paragraph('<font size="8" color="#6A6A66">CATEGORY</font>', s("th", alignment=TA_LEFT)),
            Paragraph('<font size="8" color="#6A6A66">SEVERITY</font>', s("th", alignment=TA_CENTER)),
            Paragraph('<font size="8" color="#6A6A66">EVIDENCE</font>', s("th", alignment=TA_LEFT)),
        ]
        owasp_rows = [owasp_header]
        for f in owasp_findings:
            sev = f.get("severity", "low")
            sc = sev_color.get(sev, _OLIVE)
            evidence_text = ", ".join(f.get("evidence", [])[:3])
            owasp_rows.append([
                Paragraph(
                    f'<font name="Helvetica-Bold" size="9" color="#1A1A18">{f["id"]}: {f["title"]}</font><br/>'
                    f'<font size="7" color="#6A6A66">{f["description"][:80]}…</font>',
                    s("td", leading=14),
                ),
                Paragraph(
                    f'<font name="Helvetica-Bold" size="8" color="{sc.hexval()}">{sev.upper()}</font>',
                    s("td", alignment=TA_CENTER),
                ),
                Paragraph(
                    f'<font size="7" color="#6A6A66">{evidence_text}</font>',
                    s("td"),
                ),
            ])
        col_w = W - 2 * M
        owasp_tbl = Table(owasp_rows, colWidths=[col_w * 0.45, col_w * 0.13, col_w * 0.42])
    ...
    story.append(Paragraph(
        ...
        '— Static analysis only. Results are indicative, not legal advice.  '
        'OWASP Mobile Top 10 (2024) · owasp.org/www-project-mobile-top-10</font>',
