...
def get_permission_classification_prompt(permission: str, app_category: str) -> str:
    ...
    Returns:
        A full prompt string ready to send to Groq/Llama
    """
    return f"""
You are a mobile security expert specialized in OWASP Mobile Top 10 and GDPR compliance.
Classify this Android permission for a {app_category} app.

Permission: {permission}

Think step by step:
1. What does this permission allow the app to access?
2. Is it logically required for a {app_category} app?
3. Which OWASP Mobile Top 10 (2024) category does it relate to?
   M1=Credential Usage, M2=Supply Chain, M3=Auth, M4=Validation,
   M5=Communication, M6=Privacy Controls, M7=Binary Protections,
   M8=Misconfiguration, M9=Data Storage, M10=Cryptography
4. Does it raise a GDPR data minimisation concern?
...
Examples:
- CAMERA for QR scanner → NECESSARY, LOW, M6
- READ_CONTACTS for flashlight → EXCESSIVE, HIGH, M6+M9
- LOCATION for food delivery → NECESSARY, MEDIUM, M6
- READ_CALL_LOG for calculator → EXCESSIVE, CRITICAL, M6+M9
"""
...
def get_checklist_prompt(app_category: str, permissions: list, trackers: list) -> str:
    """
    Builds the prompt to generate a sector-specific compliance checklist
    aligned with OWASP Mobile Top 10 (2024) and GDPR.
    ...
    """
    return f"""
You are a mobile security and GDPR/CCPA compliance expert.
Generate a compliance checklist for a {app_category} app, aligned with:
- OWASP Mobile Top 10 (2024): M1-M10
- GDPR / HIPAA / COPPA depending on the sector

App context:
- Detected permissions: {permissions}
- Detected trackers/SDKs: {trackers}
...
Rules:
- Each item must reference either a GDPR article OR an OWASP Mobile category (or both).
- Mark items FAIL when the detected permissions/trackers directly violate the check.
- Generate between 10 and 14 checklist items.
- Cover OWASP categories M2, M5, M6, M8, M9 where relevant to the detected data.
"""